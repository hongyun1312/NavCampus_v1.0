...
import com.hongyun.navcampus.system.mapper.VerificationCodeMapper;
...
public class VerificationService {
    ...
    public boolean verifyEmail(String username, String email, String code) {
        User u = user(username);
        return verificationCodeMapper.findTopByUserIdAndTypeAndTargetOrderByExpiresAtDesc(u.getId(), VerificationCode.CodeType.EMAIL, email)
    ...
    public boolean verifyPhone(String username, String phone, String code) {
        User u = user(username);
        return verificationCodeMapper.findTopByUserIdAndTypeAndTargetOrderByExpiresAtDesc(u.getId(), VerificationCode.CodeType.PHONE, phone)
                .filter(vc -> !vc.isVerified() && vc.getExpiresAt().isAfter(LocalDateTime.now()) && vc.getCode().equals(code))
                .map(vc -> {
                    vc.setVerified(true);
                    verificationCodeMapper.save(vc);
