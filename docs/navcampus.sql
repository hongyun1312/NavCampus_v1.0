...
CREATE TABLE `campus_landmark` (
  `landmark_id` bigint(20) NOT NULL COMMENT '地标唯一ID',
  `landmark_name` varchar(100) NOT NULL COMMENT '地标名称（如：一号教学楼、食堂）',
  `landmark_type` tinyint(4) NOT NULL COMMENT '地标类型：1-教学楼 2-宿舍 3-食堂 4-图书馆 5-校门 6-其他',
  `address` varchar(200) DEFAULT NULL COMMENT '详细地址',
  `description` text COMMENT '地标描述、开放时间、楼层分布',
  `node_id` bigint(20) NOT NULL COMMENT '关联路网节点ID（对应campus_road_node表）',
...
CREATE TABLE `campus_road_edge` (
  `edge_id` bigint(20) NOT NULL COMMENT '边唯一ID',
  `start_node_id` bigint(20) NOT NULL COMMENT '起点节点ID（关联campus_road_node）',
  `end_node_id` bigint(20) NOT NULL COMMENT '终点节点ID（关联campus_road_node）',
  `distance` decimal(8,2) NOT NULL COMMENT '边的长度（米），路径权重核心值',
  `is_one_way` tinyint(4) DEFAULT '0' COMMENT '是否单向通行：0-双向 1-单向',
  `difficulty` tinyint(4) DEFAULT '1' COMMENT '通行难度：1-平坦 2-有台阶 3-坡度大，用于路径优化',
  `status` tinyint(4) DEFAULT '1' COMMENT '状态：0-禁用 1-正常',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `del_flag` tinyint(4) DEFAULT '0' COMMENT '逻辑删除'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='校园路网边表';
...
CREATE TABLE `campus_road_node` (
  `node_id` bigint(20) NOT NULL COMMENT '节点唯一ID',
  `node_name` varchar(100) DEFAULT NULL COMMENT '节点名称（可选）',
  `x_coordinate` decimal(10,6) NOT NULL COMMENT '3D模型X坐标',
  `y_coordinate` decimal(10,6) NOT NULL COMMENT '3D模型Y坐标',
  `z_coordinate` decimal(10,6) NOT NULL COMMENT '3D模型Z坐标',
  `is_landmark` tinyint(4) DEFAULT '0' COMMENT '是否为地标节点：0-否 1-是',
  `status` tinyint(4) DEFAULT '1' COMMENT '状态：0-禁用（施工/不可通行） 1-正常',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `del_flag` tinyint(4) DEFAULT '0' COMMENT '逻辑删除'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='校园路网节点表';
...
CREATE TABLE `sys_user` (
  ...
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `del_flag` tinyint(4) DEFAULT '0' COMMENT '逻辑删除：0-未删除 1-已删除'
...
CREATE TABLE `user_course` (
  `course_id` bigint(20) NOT NULL COMMENT '课程唯一ID',
  `user_id` bigint(20) NOT NULL COMMENT '所属用户ID（关联sys_user）',
  `course_name` varchar(100) NOT NULL COMMENT '课程名称',
  `teacher_name` varchar(20) DEFAULT NULL COMMENT '授课教师',
  `week_day` tinyint(4) NOT NULL COMMENT '星期：1-周一 2-周二 ... 7-周日',
  `start_section` tinyint(4) NOT NULL COMMENT '开始节次',
  `end_section` tinyint(4) NOT NULL COMMENT '结束节次',
  `classroom_name` varchar(50) NOT NULL COMMENT '教室名称',
  `landmark_id` bigint(20) NOT NULL COMMENT '对应教室地标ID（关联campus_landmark）',
  `start_week` tinyint(4) DEFAULT '1' COMMENT '开始周次',
  `end_week` tinyint(4) DEFAULT '18' COMMENT '结束周次',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `del_flag` tinyint(4) DEFAULT '0' COMMENT '逻辑删除'
...
--
ALTER TABLE `campus_landmark`
  MODIFY `landmark_id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '地标唯一ID', AUTO_INCREMENT=50;

--
-- 使用表AUTO_INCREMENT `campus_road_edge`
--
ALTER TABLE `campus_road_edge`
  MODIFY `edge_id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '边唯一ID', AUTO_INCREMENT=35;

--
-- 使用表AUTO_INCREMENT `campus_road_node`
--
ALTER TABLE `campus_road_node`
  MODIFY `node_id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '节点唯一ID', AUTO_INCREMENT=21;

--
-- 使用表AUTO_INCREMENT `sys_user`
--
ALTER TABLE `sys_user`
  MODIFY `user_id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '用户唯一ID', AUTO_INCREMENT=2;

--
-- 使用表AUTO_INCREMENT `user_course`
--
ALTER TABLE `user_course`
  MODIFY `course_id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '课程唯一ID';
COMMIT;
